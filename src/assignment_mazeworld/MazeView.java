...
//		Color colors[] = { Color.LIGHTGRAY, Color.WHITE };
	...
	public ArrayList<Node> changeBeliefState(HashSet<int[]> beliefState) {
		
		int radius = (int)(pixelsPerSquare * .4);
		
		ArrayList<Node> pieceList = new ArrayList<Node>();
		
		int[] possiblePosition;
		Circle piece;
		
		Iterator<int[]> beliefStateIterator = beliefState.iterator();
		
		while (beliefStateIterator.hasNext()) {
			possiblePosition = beliefStateIterator.next();
			piece = new Circle(squareCenterX(possiblePosition[0]), squareCenterY(possiblePosition[1]), radius);
